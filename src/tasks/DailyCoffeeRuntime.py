...
@dataclass
class DailyCoffeeRuntimeResult:
    ok: bool
    skip_reason: str = ""
    real_purchase_performed: bool = False
    income_claimed: bool = False
...
class DailyCoffeeRuntime:
    """Runtime flow for the City Tycoon coffee shop.

    The daily activity card "前往" remains the preferred entry. F5 City Tycoon is
    a fallback/direct entry for probes and for cases where the daily card is not
    visible.
    """

    # City Tycoon is a 3D map and the one-cafe hotspot can require a focused
    # click inside the glowing disk rather than on the OCR label. Try a small
    # bounded cluster around the visible disk and stop as soon as the shop opens.
    ...
    COFFEE_POINT_POSITION = COFFEE_POINT_CANDIDATES[0]
    ...
    COFFEE_SUPPLY_BLOCKER_TEXTS = ("库存提示", "缺少", "方斯不足", "不足", "失败", "无法", "已满", "上限")
    ...
    def run(self, card=None):
        ...
        if not self.open_coffee_shop(card):
            return DailyCoffeeRuntimeResult(False, "未能进入一咖舍界面", selected_actions=self.actions)
        if self.pending_supply_error:
            return DailyCoffeeRuntimeResult(False, self.pending_supply_error, selected_actions=self.actions)

        income_claimed = self.claim_income_if_present()
        if self.is_income_report_popup():
            self.actions.append("income_popup_not_closed_after_claim")
            return DailyCoffeeRuntimeResult(
                False,
                "营收报告弹窗未关闭，未进入商品/补货流程",
                income_claimed=income_claimed,
                selected_actions=list(self.actions),
            )
        ...
        purchase_result = self.replenish_supply()
        if not purchase_result.ok:
            return DailyCoffeeRuntimeResult(
                False,
                purchase_result.skip_reason,
                income_claimed=income_claimed,
                selected_options=list(self.selected_options),
                selected_actions=list(self.actions),
            )
        ...
        return DailyCoffeeRuntimeResult(
            True,
            real_purchase_performed=purchase_result.real_purchase_performed,
            income_claimed=income_claimed,
            selected_options=list(self.selected_options),
            selected_actions=list(self.actions),
        )
    ...
    def open_coffee_shop(self, card=None):
        ...
        if self.ui.has_ensure_daily_main():
            self.actions.append("ensure_world_before_city_tycoon")
            try:
                self.ui.ensure_daily_main()
            except Exception as exc:
                self.actions.append(f"ensure_world_failed:{exc!r}")
    ...
    def optimize_products(self):
        ...
        if entry_slot is None or entry_slot.target is None:
            ...
            self.product_switch_error = "未检测到可编辑商品入口"
            return
    ...
    def replenish_supply(self):
        if self.pending_supply_completed:
            ...
            return DailyCoffeeRuntimeResult(
                True,
                real_purchase_performed=not self._dry_run(),
                selected_actions=list(self.actions),
            )

        active_seconds = self.current_business_seconds()
        if self._recent_supply_active(active_seconds) and not self._force_supply_purchase():
            self.actions.append(f"supply_recently_active_not_needed:{active_seconds}")
            return DailyCoffeeRuntimeResult(
                True,
                "supply_recently_active_not_needed",
                selected_actions=list(self.actions),
            )
        if active_seconds and self._force_supply_purchase():
            self.actions.append(f"supply_recently_active_force_purchase:{active_seconds}")
        ...
        if supply_target is None:
            ...
            return DailyCoffeeRuntimeResult(
                True,
                "supply_not_needed_or_not_found",
                selected_actions=list(self.actions),
            )
        if not self.open_supply_popup(supply_target):
            return DailyCoffeeRuntimeResult(False, "未检测到原料库存补货界面")

        configured_duration = self._supply_duration()
        if not DailyCoffeePlanner.is_allowed_duration(configured_duration):
            allowed = "/".join(DailyCoffeePlanner.ALLOWED_DURATIONS)
            return DailyCoffeeRuntimeResult(False, f"补货时长必须是固定选项之一: {allowed}")
        last_error = ""
        for index, duration in enumerate(self._supply_duration_attempts(configured_duration)):
            duration_target = self.find_text_box(duration, self.COFFEE_SUPPLY_POPUP_REGION)
            if duration_target is None:
                if index == 0:
                    return DailyCoffeeRuntimeResult(False, f"未检测到{duration}补货选项，停止购买")
            ...
            if buy_target is None:
                return DailyCoffeeRuntimeResult(False, "未检测到补货购买按钮，停止购买")
            ...
            if verify_error:
                ...
                if index < len(self._supply_duration_attempts(configured_duration)) - 1:
                    self.actions.append(f"supply_duration_blocked:{duration}:{verify_error}")
                    self.close_popup()
                    self.wait_for_supply_popup()
                    continue
                self.close_popup()
                return DailyCoffeeRuntimeResult(False, last_error, selected_actions=list(self.actions))
            self.close_popup()
            return DailyCoffeeRuntimeResult(
                True,
                real_purchase_performed=not self._dry_run(),
        ...
        return DailyCoffeeRuntimeResult(
            False,
            last_error or "补货材料或库存不足，所有固定时长均未补货",
            selected_actions=list(self.actions),
        )
    ...
    def select_coffee_from_tycoon(self):
        ...
        if label is not None:
            ...
            # The OCR label sits above the glowing entry disk. Click lower,
            # inside the actual yellow interactive area.
        ...
        for index, box in enumerate(boxes):
            ...
            options.append(
                CoffeeFoodOption(
                    ...
                    category=category,
                    trend_match=bool(category and category == self.detect_trend_category()),
                    visible_order=index,
                    target=box,
                )
            )
    ...
    def _should_switch_product(self, slot, option):
        current = slot.options[0] if slot.options else None
        if current is None or current.price_value is None:
            return True
        if option.price_value is None:
            return False
        return option.price_value > current.price_value
    ...
    def _desired_product_options(self, options, count, trend_category):
        return self._desired_product_options_from_ranked(self._rank_product_options(options, trend_category), count)
    ...
    def _rank_product_options(self, options, trend_category):
        ...
        return sorted(
            ...
            key=lambda option: (
                ...
                not DailyCoffeePlanner._matches_trend(option, trend_category),
    ...
    def _target_product_count(self, slots):
        configured = self.ui.config.get("coffee_product_target_slots", 0)
        try:
    ...
    def _product_option_is_reachable(self, candidate, allow_price_fallback=True):
        option, scrolled_steps = self._find_visible_product_option(candidate, allow_price_fallback=allow_price_fallback)
        if scrolled_steps:
            self.reset_product_options_scroll(steps=scrolled_steps)
        return option is not None

    def _click_product_option(self, candidate, action, allow_price_fallback=True):
        option, _ = self._find_visible_product_option(candidate, allow_price_fallback=allow_price_fallback)
        if option is None and self._product_scrolls():
            self.reset_product_options_scroll(steps=self._product_scrolls())
            option, _ = self._find_visible_product_option(candidate, allow_price_fallback=allow_price_fallback)
        if option is None or option.target is None:
            self.actions.append(f"{action}_not_found:{candidate.identity}")
            return False
        self._click(option.target, f"{action}:{option.identity}")
        return True

    def _find_visible_product_option(self, candidate, allow_price_fallback=True):
        page_options = self._dedupe_food_options(self.collect_product_options())
        option = self._matching_visible_product_option(
            page_options,
            candidate,
        ...
        for step in range(1, self._product_scrolls() + 1):
            self.scroll_product_options(page_options, steps=1)
            page_options = self._dedupe_food_options(self.collect_product_options())
            option = self._matching_visible_product_option(
                page_options,
            ...
            if option is not None:
                return option, step
        return None, self._product_scrolls()
    ...
    @staticmethod
    def _identity_char_overlap(left_key, right_key):
        left_chars = {char for char in str(left_key or "") if char.isalnum()}
        right_chars = {char for char in str(right_key or "") if char.isalnum()}
    ...
    def complete_pending_supply_delivery_if_present(self):
        ...
        if not any("送货上门" in text or "确认花费" in text or "库存提示" in text for text in texts):
            return False
        if not any("送货上门" in text or "确认" in text for text in texts):
            return False
        verify_error = self.finish_home_delivery_flow(
    ...
    def is_tycoon_panel(self):
        if self.is_coffee_shop_panel():
            return False
        texts = [self.box_text(box) for box in self.ocr_region(self.COFFEE_PANEL_REGION)]
        # ASCII "TYCOON" appears during the transition before the map can be
        # clicked. Wait for Chinese map/location markers before treating the
        # panel as interactive.
        return any(marker in text for text in texts for marker in self.TYCOON_TEXT_MARKERS)
    ...
    def ocr_region(self, region):
        try:
            result = self.ui.ocr_ui(*region, frame=self._fresh_frame())
        except Exception:
            return []
    ...
    def _operate_click(self, click):
        if not self.ui.operate(click, block=True):
            click()
    ...
    def _send_key(self, key, action):
        ...
        if self._dry_run():
            return
        try:
            self.ui.send_key(key, after_sleep=0)
            self._settle_after_key()
        except Exception:
            self.ui.send_foreground_key(key, after_sleep=0)
            self._settle_after_key()
    ...
    def _send_foreground_key(self, key, action):
        ...
        if self._dry_run():
            return
        if self.ui.has_send_foreground_key():
            self.ui.send_foreground_key(key, after_sleep=0)
            self._settle_after_key()
            return
        try:
    ...
    def _key_settle_seconds(self):
        try:
            return max(
                0.0,
                float(self.ui.config.get("coffee_key_settle_seconds", self.COFFEE_KEY_SETTLE_SECONDS) or 0.0),
            )
        except (TypeError, ValueError):
    ...
    def _screen_height(self):
        return self.ui.screen_height(default=1600)

    def _dry_run(self):
        return bool(self.ui.config.get("coffee_dry_run", False))

    def _force_supply_purchase(self):
        return bool(self.ui.config.get("coffee_force_supply_purchase", False))

    def _max_supply_slots(self):
        try:
            return max(0, int(self.ui.config.get("coffee_max_supply_slots", 0) or 0))
        except (TypeError, ValueError):
            return 0

    def _product_scrolls(self):
        try:
            configured = self.ui.config.get(
    ...
    def _supply_duration(self):
        duration = self.ui.config.get("coffee_supply_duration", "24小时") or "24小时"
        return DailyCoffeePlanner.normalize_duration(duration)

    @staticmethod
    def _supply_duration_attempts(duration):
        allowed = list(DailyCoffeePlanner.ALLOWED_DURATIONS)
        try:
