...
class BagelAITools(NTEOneTimeTask, BaseNTETask):
    ...
    def reply_helper(self):
        ...
        self.sleep(0.20)
        if (post_title_text == self.nowview_post and self.nowview_poster == poster_name_text) or (post_title_text in self.interacted_posts):
            self.sleep(0.50)
            return False
        self.sleep(0.20)
    ...
    def select_latest_photos(self, photo_new_count=1, photo_total=1):
        ...
        try:
            ...
            self.log_info(f"接收到非法的 photo_total: {photo_total} (类型: {type(photo_total).__name__})，已强制启用安全默认值 1")
            photo_total = 1
        # 越界输入归正
        if photo_total > 36:
            photo_total = 36
        ...
        photo_grid_locations = (
            (0.15, 0.25), (0.38, 0.25), (0.62, 0.25), (0.85, 0.25), # 第一排 1-4
            (0.15, 0.50), (0.38, 0.50), (0.62, 0.50), (0.85, 0.50), # 第二排 5-8
            (0.15, 0.75), (0.38, 0.75), (0.62, 0.75), (0.85, 0.75), # 第三排 9-12
        )
        if scroll_times > 0:
    ...
    def process_camera_action(self, action='clear_album', number=5):
        ...
        try:
            ...
            self.log_info(f"接收到非法的 number: {number} (类型: {type(number).__name__})，已强制启用默认值 5")
            number = 5
        ...
        if action == 'clear_album':
            ...
            for i in range(number):
                ...
                photo_grid_locations = (
                    ...
                    (0.15, 0.50), (0.38, 0.50), (0.62, 0.50), (0.85, 0.50), # 4-7
                    (0.15, 0.75), (0.38, 0.75), (0.62, 0.75), (0.85, 0.75), # 8-11
                )
    ...
    def get_gallery_total(self):
        ...
        if match_prefer:
            ...
            self.log_info(f"[匹配失败] 未能找到相册相片数，启用安全值 1 张，ocr结果为 '{full_ocr_text}'")
    ...
    def find_area(self, area="reply_area", action=None):
        ...
        if action == "click":
            ...
            text_area = self.wait_ocr(*ocr_area, match=match_regex, time_out=30, raise_if_not_found=True)
